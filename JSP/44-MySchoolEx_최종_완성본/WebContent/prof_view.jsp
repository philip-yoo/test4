...
<body>
	<div class="container">
		...
		<ul class="list-group">
			<li class="list-group-item"> 교수번호 : <%=item.getProfno()%>
				<a style="float:right; color:#FF0000;" href="prof_update_profno.jsp?
				profno=<%=item.getProfno()%>&name=<%=item.getName()%>&userid=<%=item.getUserid()%>&position=<%=item.getPosition()%>
				&sal=<%=item.getSal()%>&hiredate=<%=item.getHiredate()%>&comm=<%=item.getComm()%>&deptno=<%=item.getDeptno()%>" >
					수정하기
				</a>
			</li>
			...
			<li class="list-group-item"> 보직 : <%=item.getPosition()%>
				<a style="float:right; color:#FF0000;" href="prof_update_position.jsp?
				profno=<%=item.getProfno()%>&name=<%=item.getName()%>&userid=<%=item.getUserid()%>&position=<%=item.getPosition()%>
				&sal=<%=item.getSal()%>&hiredate=<%=item.getHiredate()%>&comm=<%=item.getComm()%>&deptno=<%=item.getDeptno()%>" >
					수정하기
				</a>
			...
			<li class="list-group-item"> 월급(만원) : <%=item.getSal()%>
				<a style="float:right; color:#FF0000;" href="prof_update_sal.jsp?
				profno=<%=item.getProfno()%>&name=<%=item.getName()%>&userid=<%=item.getUserid()%>&position=<%=item.getPosition()%>
				&sal=<%=item.getSal()%>&hiredate=<%=item.getHiredate()%>&comm=<%=item.getComm()%>&deptno=<%=item.getDeptno()%>" >
					수정하기
				</a>
			...
			<li class="list-group-item"> 보직수당(만원) : <%=item.getComm()%>
				<a style="float:right; color:#FF0000;" href="prof_update_comm.jsp?
				profno=<%=item.getProfno()%>&name=<%=item.getName()%>&userid=<%=item.getUserid()%>&position=<%=item.getPosition()%>
				&sal=<%=item.getSal()%>&hiredate=<%=item.getHiredate()%>&comm=<%=item.getComm()%>&deptno=<%=item.getDeptno()%>" >
					수정하기
				</a>
