...
//			--> jdbc:mysql://localhost:3306/myschool?&characterEncoding = ytf8&serverTimezone=UTC
...
//			접속 과정에서 예외처리가 요구된다.
			try {
//				mysql jdbc의 드라이버 클래스를 로딩하여 DriverManager클래서에 등록한다.
...
//				성공시 메시지 출력
				System.out.println("≡≡≡ DATASASE Connect Success ≡≡≡");

			} catch (ClassNotFoundException e) {

//				실패시 메시지와 에러 내용 출력
				System.out.println("≡≡≡ DATASASE Connect Fail ≡≡≡");
...
//				실패시 메시지와 에러 내용 출력
	...
	public void close() {
		if (conn != null) {
			...
			try {
				conn.close();
				System.out.println("=== DATABASE Diconnect Success ===");
			} catch (Exception e) {
				System.out.println("=== DATABASE Diconnect Fail===");
				System.out.println(e.getMessage());
			}
			conn = null;
		}
