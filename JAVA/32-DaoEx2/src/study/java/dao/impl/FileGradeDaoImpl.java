...
//		CSV파일 읽기
		String source = FileHelper.getIsntance().readString("res/grade.json", "euc-kr");
...
//	읽은 내용이 있다면 리턴할 List를 할당
		...
		for (int i = 0; i < data.length; i++) {
			...
			int kor = Integer.parseInt(line_data[0]);
			int eng = Integer.parseInt(line_data[0]);
			int math = Integer.parseInt(line_data[0]);
