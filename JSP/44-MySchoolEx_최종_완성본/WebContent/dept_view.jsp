...
<body>
	<div class="container">
		...
		<p class="text-right">
			<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="dept_delete.jsp?deptno=<%=deptno %>" class="btn btn-primary">삭제하기</a>
			<a href="dept_list.jsp" class="btn btn-primary"> 목록으로 </a>
		</p>
		<ul class="list-group">
			<li class="list-group-item"> 학과번호 : <%=item.getDeptno()%>
				<a style="float:right; color:#FF0000;" href="dept_update_deptno.jsp?deptno=<%=item.getDeptno()%>&dname=<%=item.getDname()%>&loc=<%=item.getLoc()%>" >
					수정하기
				</a>
			</li>
			
			<li class="list-group-item"> 학과명 : <%=item.getDname()%>
				<a style="float:right; color:#FF0000;" href="dept_update_dname.jsp?deptno=<%=item.getDeptno()%>&dname=<%=item.getDname()%>&loc=<%=item.getLoc()%>" >
					수정하기
				</a>
			</li>
			
			
			<li class="list-group-item"> 장소 : <%=item.getLoc()%>
				<a style="float:right; color:#FF0000;" href="dept_update_loc.jsp?deptno=<%=item.getDeptno()%>&dname=<%=item.getDname()%>&loc=<%=item.getLoc()%>" >
					수정하기
				</a>
			</li>
