...
//		회원가입시에 입력한 내용을 가정한 변수값들
		...
		/** 정규표현식을 사용한 입력값 형식검사 */
		if (!RegexHelper.getIsntance().isKor(name)) {
			System.out.println("이름은 한글로 입력해 주세요.");
			return;
		}
		if (!RegexHelper.getIsntance().isNum(age)) {
			System.out.println("나이는 숫자로만 입력해 주세요.");
			return;
		}
		if (!RegexHelper.getIsntance().isEmail(email)) {
			System.out.println("이메일이 잘못된 형식입니다.");
			return;
		}
		if (!RegexHelper.getIsntance().isCellPhone(phone)) {
			System.out.println("핸드폰번호가잘못된 형식 입니다.");
			return;
		}
